...
#Path
path		= cd.CIFAR10['Path']
#Check for data
if(os.listdir(path) != []):
    print('CIFAR10 Data Present.')
else:
    print('CIFAR10 Data Not Present.')
    quit()
#Image Properties
hsize 		= cd.CIFAR10['H']
vsize 		= cd.CIFAR10['V']
colours 	= cd.CIFAR10['Colours'] 
cnumb 		= cd.CIFAR10['Classes'] 
axes 		= cd.CIFAR10['Axes']
#Size of dataset
trainsize 	= cd.CIFAR10['Train']
testsize 	= cd.CIFAR10['Test']
...
with tf.variable_scope('Cifar10_Data'):
    ...
    g1,g2 = tf.train.shuffle_batch([traincifarsets[0],traincifarsets[1]], 
		batchsize, num_threads=1, capacity = 50000, enqueue_many=True, 
...
def spread():
	trlab = traincifarsets[1]
	telab = testcifarsets[1]
